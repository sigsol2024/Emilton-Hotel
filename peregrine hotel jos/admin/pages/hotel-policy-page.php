<?php
/**
 * Hotel Policy Page Editor
 */

$pageTitle = 'Hotel Policy Page Editor';
require_once __DIR__ . '/../includes/config.php';
require_once __DIR__ . '/../includes/auth.php';
require_once __DIR__ . '/../includes/header.php';

// Get all hotel policy page sections
try {
    $stmt = $pdo->prepare("SELECT * FROM page_sections WHERE page = 'hotel-policy' ORDER BY section_key");
    $stmt->execute();
    ...
    error_log("Hotel policy page editor error: " . $e->getMessage());
    $sectionsArray = [];
}
...
<form id="hotelPolicyPageForm">
    ...
    <div class="card">
        ...
        <div style="padding: 20px;">
            <div class="form-group">
                ...
                <input type="text" id="hero_title" name="hero_title" value="<?= sanitize($sectionsArray['hero_title'] ?? 'Hotel Policy') ?>">
    ...
    <div class="card">
        <div class="card-header">
            <h2>Hotel Policy Content</h2>
        </div>
        <div style="padding: 20px;">
            <div class="form-group">
                <label for="main_content">Policy Content</label>
                <textarea id="main_content" name="main_content" rows="20" style="font-family: monospace; font-size: 14px;"><?= htmlspecialchars($sectionsArray['main_content'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                <p class="form-help">HTML allowed. You can format the policy content using HTML tags like &lt;ul&gt;, &lt;li&gt;, &lt;p&gt;, etc.</p>
            </div>
        </div>
    </div>
    
    <div style="margin-top: 20px;">
        <button type="submit" class="btn btn-primary">Save Hotel Policy Page</button>
...
document.getElementById('hotelPolicyPageForm').addEventListener('submit', function(e) {
    ...
    Promise.all(promises).then(results => {
        ...
        if (allSuccess) {
            if (typeof showToast === 'function') {
                showToast('Hotel Policy page content saved successfully', 'success');
            }
        } else {
            if (typeof showToast === 'function') {
