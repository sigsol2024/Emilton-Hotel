...
<div class="card">
    ...
    <form id="roomForm" method="POST" style="padding: 20px;">
            ...
            <div id="main_image_preview" class="image-preview" style="margin-top: 10px; <?= $room['main_image'] ? 'display: block;' : 'display: none;' ?>">
                <?php if (!empty($room['main_image'])): ?>
                    ...
                    if (strpos($mainImage, 'http://') === 0 || strpos($mainImage, 'https://') === 0) {
                        $existingImageUrl = $mainImage;
                    } else {
                        // It's a relative path, prepend SITE_URL
                        $existingImageUrl = SITE_URL . ltrim($mainImage, '/');
                    }
                    ?>
                    <img id="main_image_img"
                         src="<?= htmlspecialchars($existingImageUrl, ENT_QUOTES, 'UTF-8') ?>"
        ...
        <!-- Rating & Size Section -->
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
            <h3 style="margin-bottom: 15px; font-size: 18px; font-weight: bold;">Rating & Size</h3>
            <div class="form-row">
                <div class="form-group">
                    <label for="rating_score">Rating Score (e.g., 5.0)</label>
                    <input type="number" id="rating_score" name="rating_score" class="form-control" step="0.1" min="0" max="10" value="<?= isset($room['rating_score']) && $room['rating_score'] !== null ? $room['rating_score'] : '' ?>" placeholder="5.0">
                    <p class="form-help">Displayed with star icon on room details page</p>
                </div>
                <div class="form-group">
                    <label for="size">Size (e.g., "45 m² / 484 ft²")</label>
                    <input type="text" id="size" name="size" class="form-control" value="<?= sanitize($room['size'] ?? '') ?>" placeholder="45 m² / 484 ft²">
                    <p class="form-help">Shown in Room Highlights section</p>
                </div>
            </div>
        </div>
...
function renderGalleryImages() {
    ...
    galleryImages.forEach((path, idx) => {
        ...
        const imageSrc = (pathStr.startsWith('http://') || pathStr.startsWith('https://')) 
            ? pathStr 
            : '<?= SITE_URL ?>' + pathStr.replace(/^\/+/, '');
        wrap.innerHTML = `
            <img src="${imageSrc}" alt="Gallery image" style="width:100%;height:90px;object-fit:cover;display:block;">
